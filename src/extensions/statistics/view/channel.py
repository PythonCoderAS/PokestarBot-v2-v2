...
from .shared import PrivateMode, validate_guild_only, validate_graph_options, is_private_stat, format_stat, \
    format_stat_graph_name, format_stat_embed_label, LimitedPrivateMode, BotModes
...
class ChannelHandler(BaseStatisticsViewHandler):
    ...
    def get_queryset(self, *, before_date: Optional[date], after_date: Optional[date]) -> ValuesQuery:
        ...
        if isinstance(self.channel, Thread):
            ...
            base = base.filter(channel_id=self.channel.parent.id, thread_id=self.channel.id, guild_id=self.interaction.guild_id)
        else:
    ...
    async def get_graph_data(self, stats: list[Statistic]) -> tuple[list[str], list[int], str, str, str]:
        ...
        return names, counts, f"{self.statistic.title_word()} sent in {self.channel.name}", "User", self.statistic.label()
