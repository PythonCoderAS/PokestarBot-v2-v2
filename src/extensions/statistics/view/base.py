...
class BaseStatisticsViewHandler(ABC):
    ...
    async def get_stats(self, *, before_date: date, after_date: date) -> list[Statistic]:
        ...
        if self.private_mode in ALL_AGGREGATE:
            condition = (
                (lambda item: item["is_private"]) if self.private_mode in ALL_PRIVATE_AGGREGATE else (lambda item: True)
            )
        ...
        for value in values:
            ...
            stat.agg_count = int(
                value.get("agg_count", 0)
            )  # agg_count is generated by the aggregate_threads function and it tracks the # of aggregated threads
            stats.append(stat)
